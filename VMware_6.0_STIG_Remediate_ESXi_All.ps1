...
.DESCRIPTION
	This script will remediate ALL ESXi Hosts in the target vCenter for the following vSphere 6.0 STIG items:
	ESXI-06-000001-45
...
.EXAMPLE
   ./VMware_6.0_STIG_Remediate_ESXi_All.ps1 -vcenter vcenter.test.lab
#>
...
If($remediateServices){
	ForEach($service in ($esxiServices.GetEnumerator() | Sort Name)){
		$name = $service.name
		$value = $service.value
    	ForEach($vmhost in $vmhosts | Sort Name){
