...
.NOTES 
    ...
    Author     : Ryan Lakey & SimeonOnSecurity
    Version    : 1.1
...
Try {
    ...
    $vmhosts = Get-VMHost | Where-Object { $_.version -match "^6.7*" } | Sort-Object -Property Name -ErrorAction Stop
    ...
    If ($V94563) {
        ...
        ForEach ($vm in $vmsv) {
            ...
            If ($vm.config.extraconfig.key -contains "$settingname") {
                $currentvalue = $vm.config.extraconfig | Where-Object { $_.key -eq "$settingname" }
...
Try {
    ...
    If ($V94575) {
        ...
        ForEach ($vm in $vms) {
            ...
            $vminddisks = $vm | Get-HardDisk | Where-Object { $_.Persistence -eq "IndependentNonPersistent" }
            If ($vminddisks) {
                $vmsarray += New-Object PSObject -Property ([ordered]@{
                        "Name"      = $vm.name
...
Try {
    ...
    If ($V94615) {
        ...
        ForEach ($vm in $vms) {
            Write-ToConsole "...Checking VM $($vm.Name) for $title"
            $vmcds = $vm | Get-CDDrive | Where-Object { $_.extensiondata.connectable.connected -eq $true }
            If ($vmcds) {
                $vmsarray += New-Object PSObject -Property ([ordered]@{
                        "Name"      = $vm.name
...
Try {
    ...
    If ($V94617) {
        ...
        ForEach ($vm in $vms) {
            Write-ToConsole "...Checking VM $($vm.Name) for $title"
            $vmparrallel = $vm.config.hardware.device.deviceinfo | Where-Object { $_.Label -match "parallel" }
            If ($vmparrallel) {
                $vmsarray += New-Object PSObject -Property ([ordered]@{
                        "Name"      = $vm.name
...
Try {
    ...
    If ($V94619) {
        ...
        ForEach ($vm in $vms) {
            Write-ToConsole "...Checking VM $($vm.Name) for $title"
            $vmserial = $vm.config.hardware.device.deviceinfo | Where-Object { $_.Label -match "serial" }
            If ($vmserial) {
                $vmsarray += New-Object PSObject -Property ([ordered]@{
                        "Name"      = $vm.name
...
Try {
    ...
    If ($V94621) {
        ...
        ForEach ($vm in $vms) {
            Write-ToConsole "...Checking VM $($vm.Name) for $title"
            $vmusb = $vm.config.hardware.device.deviceinfo | Where-Object { $_.Label -match "USB con*" }
            If ($vmusb) {
                $vmsarray += New-Object PSObject -Property ([ordered]@{
                        "Name"      = $vm.name
                        "Found"     = "VM has $($vmusb.count) USB Controllers attached"
                        "Expected"  = "USB controllers do not exist except Where-Object needed to attach Smart Card readers"
                        "Severity"  = $Severity
                        "Compliant" = $false
                    })
            }
...
Try {
    ...
    If ($V94635) {
        ...
        ForEach ($vm in $vmsv) {
            Write-ToConsole "...Checking VM $($vm.Name) for $settingname"
            $dvFilters = $vm.config.extraconfig | Where-Object { $_.key -like $settingname }            
            If ($dvFilters) {
                $currentvalue = $vm.config.extraconfig | Where-Object { $_.key -like $settingname }
                $vmsarray += New-Object PSObject -Property ([ordered]@{
                        "Name"      = $vm.name
                        "Setting"   = $currentvalue.key
                        "Value"     = $currentvalue.value
...
Try {
    ...
    If ($V94725) {
        ...
        Write-ToConsole "...Checking STIG Control with Vulnerability ID:$VULID STIG ID:$STIGID Severity:$Severity with Title: $Title"
        $vcarray = @()
        $perms = Get-VIPermission | Sort-Object -Property Role | Select-Object Role, Principal, Entity, Propagate, IsGroup
...
$vmsarrayallcompliantcount = @($vmsarrayall | Where-Object { $_.Compliant -eq $true }).count
$vmsarrayallnoncompliantcount = @($vmsarrayall | Where-Object { $_.Compliant -eq $false }).count

##Virtual Machine Compliance By Severity
$vmscompliantcountcat1 = @($vmsarrayall | Where-Object { $_.Compliant -eq $true -and $_.Severity -eq "CAT I" }).count
$vmsnoncompliantcountcat1 = @($vmsarrayall | Where-Object { $_.Compliant -eq $false -and $_.Severity -eq "CAT I" }).count
$vmscompliantcountcat2 = @($vmsarrayall | Where-Object { $_.Compliant -eq $true -and $_.Severity -eq "CAT II" }).count
$vmsnoncompliantcountcat2 = @($vmsarrayall | Where-Object { $_.Compliant -eq $false -and $_.Severity -eq "CAT II" }).count
$vmscompliantcountcat3 = @($vmsarrayall | Where-Object { $_.Compliant -eq $true -and $_.Severity -eq "CAT III" }).count
$vmsnoncompliantcountcat3 = @($vmsarrayall | Where-Object { $_.Compliant -eq $false -and $_.Severity -eq "CAT III" }).count
...
$vcarrayallcompliantcount = @($vcarrayall | Where-Object { $_.Compliant -eq $true }).count
$vcarrayallnoncompliantcount = @($vcarrayall | Where-Object { $_.Compliant -eq $false }).count

##Virtual Machine Compliance By Severity
$vccompliantcountcat1 = @($vcarrayall | Where-Object { $_.Compliant -eq $true -and $_.Severity -eq "CAT I" }).count
$vcnoncompliantcountcat1 = @($vcarrayall | Where-Object { $_.Compliant -eq $false -and $_.Severity -eq "CAT I" }).count
$vccompliantcountcat2 = @($vcarrayall | Where-Object { $_.Compliant -eq $true -and $_.Severity -eq "CAT II" }).count
$vcnoncompliantcountcat2 = @($vcarrayall | Where-Object { $_.Compliant -eq $false -and $_.Severity -eq "CAT II" }).count
$vccompliantcountcat3 = @($vcarrayall | Where-Object { $_.Compliant -eq $true -and $_.Severity -eq "CAT III" }).count
$vcnoncompliantcountcat3 = @($vcarrayall | Where-Object { $_.Compliant -eq $false -and $_.Severity -eq "CAT III" }).count
