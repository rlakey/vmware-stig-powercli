<#
.SYNOPSIS
	vSphere 6.0 STIG Virtual Machine Remediation Script
	Created by: Ryan Lakey, rlakey@vmware.com 
	Provided as is and is not supported by VMware
.DESCRIPTION
	This script will remediate Virtual Machines listed in a provided text file in the target vCenter for the following vSphere 6.0 STIG items:
...
.PARAMETER vcenter
   vCenter server to run script against and remediate all Virtual Machines listed in a provided text file in that vCenter
.PARAMETER file
...
.EXAMPLE
   ./VMware_6.0_STIG_Remediate_VM_FromTextFile.ps1 -vcenter vcenter.test.lab -file C:\PowerCLI\ServerstoRemediate.txt
#>
