...
.DESCRIPTION
	This script will remediate the ESXi hosts provided in a text file in the target vCenter for some vSphere 5.x STIG items.
...
.PARAMETER file
   Path to text file and name with list of ESXi hosts to remediate.  Name must be name of ESXi host in vCenter with 1 per line and no extra spaces.
.PARAMETER cred
   This will prompt user for credentials that will be used to connect to vCenter specified.
.EXAMPLE
   ./VMware_5.x_STIG_Remediate_ESXi_FromTextFile.ps1 -vcenter vcenter.test.lab -ds mydatastore -file C:\PowerCLI\hostlist.txt
#>
...
$vmhost = Get-VMHost -Name $server | Where {$_.version -match "^5.*"}
